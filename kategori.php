...
if (isset($_POST['simpan_kategori'])) {
    ...
    $simpan = mysqli_query($koneksi, "INSERT INTO kategori (nama_kategori, deskripsi) VALUES ('$nama', '$desc')");
    if ($simpan) { header("location:kategori.php"); }
}
...
if (isset($_POST['update_kategori'])) {
    ...
    $nama = mysqli_real_escape_string($koneksi, $_POST['nama_kategori']);
    $desc = mysqli_real_escape_string($koneksi, $_POST['deskripsi']);
    $update = mysqli_query($koneksi, "UPDATE kategori SET nama_kategori='$nama', deskripsi='$desc' WHERE id_kategori='$id'");
    if ($update) { header("location:kategori.php"); }
}
...
$query_kategori = mysqli_query($koneksi, "SELECT * FROM kategori ORDER BY id_kategori DESC");
...
<head>
    ...
    <title>Kategori - Toko Sembako</title>
...
<body class="bg-[#fcfcfc]">
    ...
    <header class="bg-white border-b border-gray-200 px-8 py-3 flex justify-between items-center sticky top-0 z-40">
        ...
        <div class="flex items-center gap-6">
                ...
                <div id="dropdownNotif" class="hidden absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 z-[100] overflow-hidden text-left">
                    ...
                    <div class="max-h-80 overflow-y-auto custom-scrollbar text-left">
                        <?php if($jml_kritis > 0): 
                            ...
                            <div class="px-5 py-4 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-3 border-l pl-6 border-gray-300">
                ...
                <a href="logout.php" onclick="return confirm('Yakin ingin keluar?')" class="text-gray-900 hover:text-red-600 transition-colors">
    ...
    <nav class="bg-[#05051a] px-8 flex gap-1">
        ...
        <a href="kategori.php" class="flex items-center gap-2 px-6 py-4 text-white text-sm font-medium transition-all border-b-2 border-white bg-white/10"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path></svg> Kategori</a>
    ...
    <main class="p-8 space-y-6">
        ...
        <div class="border border-gray-900 rounded-2xl p-8 bg-white shadow-sm min-h-[400px]">
            <table class="w-full text-left whitespace-nowrap">
                ...
                <tbody>
                    <?php if ($total_kategori > 0): $no = 1; while ($row = mysqli_fetch_assoc($query_kategori)): ?>
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                            <td class="py-4 text-gray-500 font-medium"><?= $no++; ?></td>
                            <td class="py-4 font-bold text-gray-900"><?= htmlspecialchars($row['nama_kategori']); ?></td>
                            <td class="py-4 text-gray-500"><?= htmlspecialchars($row['deskripsi']); ?></td>
                            <td class="py-4 text-right">
                                <button onclick="openEditModal('<?= $row['id_kategori'] ?>', '<?= htmlspecialchars($row['nama_kategori']) ?>', '<?= htmlspecialchars($row['deskripsi']) ?>')" class="text-blue-600 font-bold mr-4 hover:underline">Edit</button>
                                <a href="kategori.php?hapus_id=<?= $row['id_kategori'] ?>" onclick="return confirm('Hapus kategori ini?')" class="text-red-600 font-bold hover:underline">Hapus</a>
                            </td>
                        </tr>
                    <?php endwhile; else: ?>
    ...
    <div id="modalKategori" class="modal fixed inset-0 flex items-center justify-center z-[100]">
        ...
        <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded-2xl shadow-2xl z-50 overflow-hidden">
            <div class="p-8">
                ...
                <form action="kategori.php" method="POST" class="mt-6 space-y-5">
